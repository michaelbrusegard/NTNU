...
class User(AbstractUser):
    ...
    """
    isCoach = models.fields.BooleanField(default=False)
    coach = models.ForeignKey(
        "self", on_delete=models.CASCADE, related_name="athletes", blank=True, null=True
    )
    specialism = models.fields.CharField(max_length=1000,default="")
...
class AthleteFile(models.Model):
    ...
    file = models.FileField(upload_to=athlete_directory_path, validators=[FileValidator(
        allowed_mimetypes='', allowed_extensions='', max_size=1024*1024*5)]
    )
